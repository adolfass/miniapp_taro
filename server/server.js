...
app.post('/api/create-invoice', async (req, res) => {
  try {
    ...
    console.log('🔍 SERVER DEBUG: Telegram API full response:', JSON.stringify(response.data, null, 2));
    console.log('🔍 SERVER DEBUG: response.data.ok:', response.data.ok);
    console.log('🔍 SERVER DEBUG: response.data.result:', response.data.result);
    console.log('🔍 SERVER DEBUG: invoice_link:', response.data.result?.invoice_link);
    ...
    // Check if invoice_link exists
    if (!response.data.result?.invoice_link) {
      console.error('❌ SERVER DEBUG: invoice_link is missing! Full result:', JSON.stringify(response.data.result, null, 2));
      return res.status(500).json({
        success: false,
...
╚════════════════════════════════════════════╝
  ...
  // Запускаем бота админки (polling)
  adminBot.startBot();
