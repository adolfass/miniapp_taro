...
show_summary() {
    ...
    echo "  - toOpencode.md (текущие инструкции для Opencode)"
    echo "  - PAYMENTS_TEST_TO_OPENCODE.md (тестирование оплаты)"
    echo "  - to_opencode_agents.md (общие инструкции)"
    ...
    echo ""
    echo -e "${YELLOW}📬 Путь для Opencode Agent:${NC}"
    echo "  /var/www/tarot-miniapp/exchange/toOpencode.md"
    echo ""
    echo -e "${YELLOW}📝 Путь для отчётов:${NC}"
    ...
    echo "  3. Прочитайте инструкции: cat exchange/toOpencode.md"
    echo "  4. Запустите тест: ./test-payment.sh"
    echo ""
}
